...
int create_unix_socket(char *path) {
    ...
    struct sockaddr_un sock_bind;
    sock_bind.sun_family = AF_UNIX;
    strcpy(sock_bind.sun_path, name);

    if(bind(sock_fd, (const struct sockaddr *) &sock_bind, sizeof(sock_bind)) == -1) {
        perror("bind");
        exit(1);
    }
    if(connect(sock_fd, (const struct sockaddr *) &sock_addr, sizeof(sock_addr)) == -1) {
